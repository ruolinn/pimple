...
if test "$PHP_PIMPLE" != "no"; then
  ...
  dnl [
  dnl   PHP_ADD_LIBRARY_WITH_PATH($LIBNAME, $PIMPLE_DIR/$PHP_LIBDIR, PIMPLE_SHARED_LIBADD)
  dnl   AC_DEFINE(HAVE_PIMPLELIB,1,[ ])
  dnl ],[
  dnl   AC_MSG_ERROR([wrong pimple lib version or lib not found])
  dnl ],[
  dnl   -L$PIMPLE_DIR/$PHP_LIBDIR -lm
  dnl ])
  dnl
  dnl PHP_SUBST(PIMPLE_SHARED_LIBADD)

  PHP_NEW_EXTENSION(pimple, pimple.c, $ext_shared,, -DZEND_ENABLE_STATIC_TSRMLS_CACHE=1)
fi
